...
# Copyright 2012-2016  Johns Hopkins University (Author: Daniel Povey) # SpecAug version modified by Harsha...specaug default settings used
# Apache 2.0
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_mfcc_${name}.JOB.log \
    ...
    compute-mfcc-feats-masks $vtln_opts $write_utt2dur_opt --verbose=2 \
  ...
  $cmd JOB=1:$nj $logdir/make_mfcc_${name}.JOB.log \
    compute-mfcc-feats-masks --time-mask=1 --frequency-mask=2 --nbr-time-mask=2 --nbr-frequency-mask=1 $vtln_opts $write_utt2dur_opt --verbose=2 \
...
echo ${frame_shift:-'0.03'} > $data/frame_shift
